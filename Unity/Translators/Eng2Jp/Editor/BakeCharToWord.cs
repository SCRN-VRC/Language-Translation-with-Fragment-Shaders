...
[ExecuteInEditMode]
public class BakeCharToWord : EditorWindow
{
    public TextAsset charList;
    public TextAsset wordMapping;
    string SavePath;

    [MenuItem("Tools/SCRN/Bake Eng CharToWord")]
    static void Init()
    {
        var window = GetWindowWithRect<BakeCharToWord>(new Rect(0, 0, 400, 250));
        window.Show();
    }

    void OnGUI()
    {
        GUILayout.Label("Bake CharToWord", EditorStyles.boldLabel);
        ...
        if (GUILayout.Button("Bake!") && charList != null && wordMapping != null) {
            string path = AssetDatabase.GetAssetPath(charList);
            int fileDir = path.LastIndexOf("/");
            SavePath = path.Substring(0, fileDir) + "/baked-charToWord.asset";
            OnGenerateTexture();
        }
    }
    ...
    void OnGenerateTexture()
    {
        ...
        foreach (var line in arrayString)
        {
            ...
            if (tokens[0][0] != oldChar)
            {
                oldChar = tokens[0][0];
                Debug.Log(oldChar + " " + i.ToString());
            }
            ...
            float val = float.Parse(tokens[1], CultureInfo.InvariantCulture.NumberFormat);
            tex.SetPixel(x + 1, y + 1, new Color(val, 0, 0, 0));
