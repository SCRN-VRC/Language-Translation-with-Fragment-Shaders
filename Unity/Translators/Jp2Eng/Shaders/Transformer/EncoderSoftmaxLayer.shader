Shader "Translator/Jp2Eng/Encoder/SoftmaxLayer"
{
    Properties
    {
        ...
        Pass
        {
            ...
            float frag (v2f i) : SV_Target
            {
                ...
                if (TLState == ST_ENCODER && laC == lid)
                {
                    ...
                    {
                        s += exp(getLayer(_LayersTex, PRID.x, uint3(k, l, n)));
                    }
                    s = exp(getLayer(_LayersTex, PRID.x, uint3(k, l, m))) / s;
                    col = s;
                }
                //if (k == 7 && l == 21 && m == 11) buffer[0] = col;
                return col;
            }
            ENDCG
        }
