Shader "Translator/Jp2Eng/Decoder/AttentionQKLayer"
{
    Properties
    {
        ...
        Pass
        {
            ...
            float frag (v2f i) : SV_Target
            {
                ...
                if (TLState == ST_DECODER && laC == lid)
                {
                    ...
                    {
                        s += getSplitHeads(_LayersTex, PRID.x, uint3(k, l, n)) *
                            getSplitHeads(_LayersTex, PRID.y, uint3(k, m, n));
                    }

                    s /= sqrt(WBLID.z);

                    if (PRID.w == 0)
                        s += getDecoderMask(_LayersTex, uint2(l, m));
                    else
                        s += getEncoderMask(_LayersTex, uint2(l, m));

                    col = s;
                    //if (WBLID.w == 18 && loC == 0 && k == 3 && l == 5 && m == 4) buffer[0] = s;
                }
                return col;
            }
            ENDCG
