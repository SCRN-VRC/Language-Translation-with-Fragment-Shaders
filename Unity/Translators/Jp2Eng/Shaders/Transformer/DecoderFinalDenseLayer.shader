Shader "Translator/Jp2Eng/Decoder/FinalDenseLayer"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float frag (v2f i) : SV_Target
            {
                ...
                {
                    uint l = px.x + px.y * 1024;
                    uint k = maxDim - 1;
                    if (l >= 44337) return 0.0;
                    ...
                    for (uint m = 0; m < WBLID.z; m++)
                    {
                        s += getLayer(_LayersTex, PRID.x, uint2(k, m)) *
                            getWeightsFinal(_WeightsTex, uint2(m, l));
                    }

                    s += getBiasFinal(_WeightsTex, l);
                    col = s;
                    //if (l == 1) buffer[0] = float4(k, s, 0, 0);
                }
                return col;
            }
            ENDCG
