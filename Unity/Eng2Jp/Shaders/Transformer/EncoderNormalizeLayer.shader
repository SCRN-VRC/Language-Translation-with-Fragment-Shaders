Shader "Translator/Eng2Jp/Encoder/NormalizeLayer"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            UNITY_INSTANCING_BUFFER_START(Props)
                ...
                UNITY_DEFINE_INSTANCED_PROP(uint, _LoopOffset)
            ...
            float frag (v2f i) : SV_Target
            {
                ...
                uint LOFF = UNITY_ACCESS_INSTANCED_PROP(Props, _LoopOffset);
                ...
                if (TLState == ST_ENCODER && laC == lid)
                {
                    uint l = px.x;
                    uint k = px.y;
                    float s = 0.0;

                    // center and scale around mean/variance
                    s = (getLayer(_LayersTex, PRID.x, uint2(k, l)) + getLayer(_LayersTex, PRID.y, uint2(k, l)) - 
                        getLayer(_LayersTex, PRID.z, uint2(k, 0))) / sqrt(getLayer(_LayersTex, PRID.w, uint2(k, 0)) + epsilon);
                    
                    // gamma + beta
                    s = s * getConst(_WeightsTex, WBLID.x + LOFF * loC, uint2(l, 0)) +
                        getConst(_WeightsTex, WBLID.y + LOFF * loC, uint2(l, 0));
                    col = s;
                    //if (WBLID.w == 14 && loC == 5 && k == 21 && l == 500) buffer[0] = s;
                }
                return col;
            }
            ENDCG
