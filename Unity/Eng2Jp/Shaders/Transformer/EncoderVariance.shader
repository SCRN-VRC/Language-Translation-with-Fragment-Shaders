Shader "Translator/Eng2Jp/Encoder/Variance"
{
    Properties
    {
        ...
        Pass
        {
            ...
            float frag (v2f i) : SV_Target
            {
                ...
                if (TLState == ST_ENCODER && laC == lid)
                {
                    ...
                    {
                        var += pow(getLayer(_LayersTex, PRID.x, uint2(l, m)) +
                            getLayer(_LayersTex, PRID.y, uint2(l, m)) -
                            getLayer(_LayersTex, PRID.z, uint2(l, 0)), 2);
                    }
                    var /= float(WBLID.z);
                    col = var;
                }

                //if (l == 21) buffer[0] = var;
                return col;
            }
            ENDCG
        }
