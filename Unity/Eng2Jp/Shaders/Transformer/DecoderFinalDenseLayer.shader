Shader "Translator/Eng2Jp/Decoder/FinalDenseLayer"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float frag (v2f i) : SV_Target
            {
                ...
                if (TLState == ST_DEC_FINAL)
                {
                    uint l = (px.x % 57) + (px.y % 57) * 57;
                    uint k = (px.x / 57) + (px.y / 57) * 5;
                    if (k >= maxDim || l >= 3229) return 0.0;
                    ...
                    for (uint m = 0; m < WBLID.z; m++)
                    {
                        s += getLayer(_LayersTex, PRID.x, uint2(k, m)) *
                            getConst(_WeightsTex, WBLID.x, uint2(m, l));
                    }

                    s += getConst(_WeightsTex, WBLID.y, uint2(l, 0));
                    col = s;
                    //if (k == 17 && l == 2) buffer[0] = s;
                }
                return col;
            }
            ENDCG
