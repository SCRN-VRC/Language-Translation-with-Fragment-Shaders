Shader "Translator/Eng2Jp/Decoder/MHALayer"
{
    Properties
    {
        ...
        Pass
        {
            ...
            float frag (v2f i) : SV_Target
            {
                ...
                if (TLState == ST_DECODER && laC == lid)
                {
                    ...
                    for (uint m = 0; m < WBLID.z; m++)
                    {
                        s += getSAT(_LayersTex, PRID.x, uint2(k, m)) *
                            getConst(_WeightsTex, WBLID.x + LOFF * loC, uint2(m, l));
                    }

                    s += getConst(_WeightsTex, WBLID.y + LOFF * loC, uint2(l, 0));
                    col = s;
                    //if (WBLID.w == 21 && loC == 0 && k == 3 && l == 99)
                    //    buffer[0] = getSAT(_LayersTex, PRID.x, uint2(3, 384));
                }
                return col;
            }
            ENDCG
