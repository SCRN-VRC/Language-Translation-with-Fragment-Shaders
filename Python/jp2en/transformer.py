...
input_vocab_size = len(token_Japanese.word_index) + 2
target_vocab_size = len(token_English.word_index) + 2
dropout_rate = 0.1
...
for epoch in range(EPOCHS):
    ...
    for i in range(int(len(Japanese)/batch_size)):
        inp = tf.convert_to_tensor(np.array(Japanese[start_pt:start_pt+batch_size]),dtype=tf.int64)
        tar = tf.convert_to_tensor(np.array(English[start_pt:start_pt+batch_size]),dtype=tf.int64)
...
def evaluate(sentence):
    ...
    sentence.append('<EOS>')
    sentence = token_Japanese.texts_to_sequences(sentence)
    # For some reason tokenizing JP returns [[1], [2], [3]] when we want [1, 2, 3]
    sentence = [sentence[i][0] for i in range(len(sentence))]
    sentence = pad([sentence], length = len(Japanese[0]))
    sentence = tf.convert_to_tensor(np.array(sentence),dtype=tf.int64)

    decoder_input = tf.convert_to_tensor(np.array(token_English.texts_to_sequences(['SOS'])),dtype=tf.int64)

    outputs = None
    for i in range(len(Japanese[0])):
        ...
        if predicted_id == token_English.texts_to_sequences(['EOS']):
...
def toJapanese(tokens):
    tokens = tokens.numpy()
    tokens = [[tokens[i]] for i in range(len(tokens))]
    tokens = token_Japanese.sequences_to_texts(tokens)
    tokens.remove('sos')
    return ' '.join(tokens)

def toEnglish(tokens):
    tokens = tokens.numpy()
    tokens = [[tokens[i]] for i in range(len(tokens))]
    tokens = token_English.sequences_to_texts(tokens)
    tokens.remove('sos')
    return ' '.join(tokens)
...
# result(5, True)
# layers_out, decoded = translate('私の名前はサラでロンドンに住んでいる')
# layers_out, decoded = translate('犬の上にノートパソコンを置く')
# layers_out, decoded = translate('ピザを屋根に投げた')
# layers_out, decoded = translate('私の顔はバターに溶けました')
layers_out, decoded = translate('ドアを開けて床に着く')
...
if 0:
    ...
    dest = './save_model/jp2eng_weights.bytes'
