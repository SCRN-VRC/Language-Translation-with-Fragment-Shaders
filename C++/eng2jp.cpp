...
public:
    ...
    eng2jp(string pathWeights, string pathText2Seq, string pathSeq2Text)
    {
        ifstream fin(pathWeights, ios::binary);
        if (!fin) {
            cout << "error opening stream" << endl;
            exit(-1);
        }

        const0 = getArray(&fin, 44337, 512);
        ...
        const254 = getArray(&fin, 512, 3229);
        const255 = getArray(&fin, 3229);

        fin.close();

        ifstream fin2(pathText2Seq);
        if (!fin2) {
        ...
        while (std::getline(fin2, line))
        {
            ...
            string key = token;
            getline(ss, token, '\t');
            uint value = stoul(token);
            engMap.insert(pair<string, uint>(key, value));
        }
        ...
        final_out = (float**)createArray(22, 3229, sizeof(float));
    ...
    ~eng2jp()
    {
        ...
        freeArray(512, 3229, const254);
        freeArray(3229, const255);
    ...
    */
    uint* text2seq(string input)
    {
        ...
        uint c = 1;
        while (getline(ss, token, ' ') && c < 21)
        {
            uint val = engMap[token];
            if (val == 0) continue; // Word not in vocabulary
            seqArray[c++] = val;
        }
        seqArray[c] = 1; // EOS
        return seqArray;
    ...
    */
    ...
    void forwardProp(string input)
    {
        ...
        for (uint k = 0; k < 22; k++) {
            thread t(&eng2jp::embeddingLayer, this, encoder_in, const0, seqIn, 512, k);
        ...
        for (int c = 0; c < 6; c++)
        {
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::denseLayer, this, encoder.lmhaQ, encoder.weights.mhaQ_w,
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::denseLayer, this, encoder.lmhaK, encoder.weights.mhaK_w,
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::denseLayer, this, encoder.lmhaV, encoder.weights.mhaV_w,
            ...
            for (uint k = 0; k < 8; k++) {
                thread t(&eng2jp::attentionQKLayer, this, encoder.lsatQK, encoder.lmhaQ,
            ...
            for (uint k = 0; k < 8; k++) {
                thread t(&eng2jp::softmaxLayer, this, encoder.lsoft, encoder.lsatQK, 22, 22, k);
                threads.push_back(move(t));
            }
            for (auto& th : threads) th.join();
            threads.clear();
            ...
            for (uint k = 0; k < 8; k++) {
                thread t(&eng2jp::attentionSVLayer, this, encoder.lsatSV, encoder.lmhaV,
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::multiHeadAttentionLayer, this, encoder.lmhaO, encoder.weights.mhaO_w,
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::normalizeLayer, this, encoder.lnorm1, encoder.lmean1, encoder.lvar1,
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::denseLayerRELU, this, encoder.lffn1, encoder.weights.ffn1_w,
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::denseLayer, this, encoder.lffn2, encoder.weights.ffn2_w,
            ...
            for (uint k = 0; k < 22; k++) {
                thread t(&eng2jp::normalizeLayer, this, encoder.lnorm2, encoder.lmean2, encoder.lvar2,
        ...
        while (predict_id != 2 && seqOutLen <= 22)
        {
            createTargetMask(seqOut, decoderTargetMask);

            // embedding + scale layer + positional encode
            for (uint k = 0; k < seqOutLen; k++) {
                thread t(&eng2jp::embeddingLayer, this, decoder_in, const97, seqOut, 512, k);
                threads.push_back(move(t));
            }
            for (auto& th : threads) th.join();
            threads.clear();
            ...
            for (int c = 0; c < 6; c++)
            {
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::denseLayer, this, decoder.lmha1Q, decoder.weights.mha1Q_w,
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::denseLayer, this, decoder.lmha1K, decoder.weights.mha1K_w,
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::denseLayer, this, decoder.lmha1V, decoder.weights.mha1V_w,
                ...
                for (uint k = 0; k < 8; k++) {
                    thread t(&eng2jp::attentionQKLayer, this, decoder.lsat1QK, decoder.lmha1Q,
                ...
                for (uint k = 0; k < 8; k++) {
                    thread t(&eng2jp::softmaxLayer, this, decoder.lsoft1, decoder.lsat1QK, seqOutLen, seqOutLen, k);
                    threads.push_back(move(t));
                }
                for (auto& th : threads) th.join();
                threads.clear();
                ...
                for (uint k = 0; k < 8; k++) {
                    thread t(&eng2jp::attentionSVLayer, this, decoder.lsat1SV, decoder.lmha1V,
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::multiHeadAttentionLayer, this, decoder.lmha1O, decoder.weights.mha1O_w,
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::normalizeLayer, this, decoder.lnorm1, decoder.lmean1, decoder.lvar1,
                ...
                // multi head attention, decoder head_q
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::denseLayer, this, decoder.lmha2Q, decoder.weights.mha2Q_w,
                ...
                // multi head attention, decoder head_k
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::denseLayer, this, decoder.lmha2K, decoder.weights.mha2K_w,
                ...
                // multi head attention, decoder head_v
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::denseLayer, this, decoder.lmha2V, decoder.weights.mha2V_w,
                ...
                // scaled attention step q * k, decoder head
                for (uint k = 0; k < 8; k++) {
                    thread t(&eng2jp::attentionQKLayer, this, decoder.lsat2QK, decoder.lmha2Q,
                ...
                // scaled attention step softmax, decoder head
                for (uint k = 0; k < 8; k++) {
                    thread t(&eng2jp::softmaxLayer, this, decoder.lsoft2, decoder.lsat2QK, seqOutLen, seqOutLen, k);
                    threads.push_back(move(t));
                }
                for (auto& th : threads) th.join();
                threads.clear();
                ...
                // scaled attention step soft * v, decoder head
                for (uint k = 0; k < 8; k++) {
                    thread t(&eng2jp::attentionSVLayer, this, decoder.lsat2SV, decoder.lmha2V,
                ...
                // multi head attention, decoder head_dense_out
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::multiHeadAttentionLayer, this, decoder.lmha2O, decoder.weights.mha2O_w,
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::normalizeLayer, this, decoder.lnorm2, decoder.lmean2, decoder.lvar2,
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::denseLayerRELU, this, decoder.lffn1, decoder.weights.ffn1_w,
                ...
                for (uint k = 0; k < 22; k++) {
                    thread t(&eng2jp::denseLayer, this, decoder.lffn2, decoder.weights.ffn2_w,
                ...
                for (uint k = 0; k < seqOutLen; k++) {
                    thread t(&eng2jp::normalizeLayer, this, decoder.lnorm3, decoder.lmean3, decoder.lvar3,
            ...
            // final dense output
            for (uint k = 0; k < seqOutLen; k++) {
                thread t(&eng2jp::denseLayer, this, final_out, const254, const255, decoder_in, 3229, 512, k);
                threads.push_back(move(t));
            }
            for (auto& th : threads) th.join();
            threads.clear();
...
int main()
{
    ...
    eng2jp translator = eng2jp(PATHWEIGHTS, PATHTEXT2SEQ, PATHSEQ2TEXT);
    translator.forwardProp(input);
